<?php
	...
	$command = new Command();
	$listCommands = $command->mt_listComands();
?>  
<!DOCTYPE html>
<html lang="en">
						...
						<tbody>
							<?php foreach ($listCommands as $command) {?>
								<tr class="odd gradeX">
									...
									<td><?php echo $command["url"];?></td>
									<td class="center"><?php echo $command["request_date"];?></td>
									<td class="center"><?php echo $command["processing_date"];?></td>
									<?php if ($command["status"] == "3"){ ?>
										<td class="center"><span class="badge bg-success">Concluído</span></td>
									<?php }else if ($command["status"] == "2"){ ?>
